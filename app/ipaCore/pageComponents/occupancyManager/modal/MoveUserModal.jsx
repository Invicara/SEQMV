...
const MoveUserModal = ({ isOpen, onClose, userInfo,isMoved, floorValue,onTextSearch,onFloorChange,assetClickType,onFetch, ...props }) => {
  ...
  const getLevels = async() => {
    ...
    if(floorValue == undefined){
      ...
      const getAllAssets = await ScriptCache.runScript(props.handler.script.getOccupancyAssets, { entityInfo: {}})
  ...
  const getRooms = async (floor) => {
    ...
    //{floor: '1 st Floor'}
    setIsLoading(true);
    const floorAssets = await ScriptCache.runScript(props.handler.config.entityData.getAssets.script, { floor })
  ...
  const editAsset = async () => {
    ...
    try {
      ScriptCache.clearCache();
      const floorAssets = await ScriptCache.runScript(props.handler.config.entityData.editUser.script, [updateToAsset.entity, updateFromAsset.entity])
