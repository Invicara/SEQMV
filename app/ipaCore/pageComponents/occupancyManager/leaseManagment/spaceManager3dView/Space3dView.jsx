...
const Space3dView = ({ selectedItems, ...props }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // setSpaceBiuldingAndLevels(buildingData)
      ...
      // setSpaceBiulding(uniqueBuildings)
      // setSpaceLevels(uniqueLevels)
  ...
  const loadAllModels = async () => {
    try {
      let spaceMapModel = await ScriptCache.runScript(props.handler.config.entityData.getSpacesMapModel.script, {});
  ...
  const onFloorChange = async (value) => {
    ...
    try {
      ...
      const floorSpaces = await ScriptCache.runScript(props.handler.config.entityData.getFilterSpaces.script, { "floor":levelName,"model":selectedModel ,"collectionInfo":selectedSpaceMapWithModel[0] })
  ...
  const onOfficeChange = async (value) => {
    ...
    try {
      ...
      // ScriptCache.clearCache();
      // const selectedSpace = await ScriptCache.runScript(props.handler.config.entityData.getSpaceByName.script, filters)
  ...
  const onFetch = async (value) => {
    ...
    try {
      ScriptCache.clearCache();
      const floorSpaces = await ScriptCache.runScript(props.handler.config.entityData.getFilterSpaces.script, filters)
  ...
  const onTextSearch = async (value) => {
    ...
    try {
      ScriptCache.clearCache();
      const floorAssets = await ScriptCache.runScript(props.handler.config.entityData.getFilterSpaces.script, { "userName": value })
