...
const OccupancyManagement = ({ selectedItems, ...props }) => {
  ...
  // useEffect(async () => {
  //   const spaceLevelData = await ScriptCache.runScript(props.handler.config.entityData.Level.script, {})
  //   setSpaceLevels(spaceLevelData);

  //   IafProj.getProjectModels(selectedItems?.selectedProject._id)
  //     .then((res) => {
  //       console.log('Model to load :------>', res)
  //       // const model = res._list.find(model => model._name == 'General Medical - Architecture')
  ...
  const onFloorChange = async (value) => {
    ...
    try {
      ...
      const floorAssets = await ScriptCache.runScript(props.handler.config.entityData.getAssets.script, { levelName })
  ...
  const onFetch = async (value) => {
    ...
    try {
      ScriptCache.clearCache();
      const floorAssets = await ScriptCache.runScript(props.handler.config.entityData.getAssets.script, filters)
  ...
  const onTextSearch = async (value) => {
    ...
    try {
      ScriptCache.clearCache();
      const floorAssets = await ScriptCache.runScript(props.handler.config.entityData.getAssets.script, { "userName": value })
